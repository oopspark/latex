...
Thus,
Any monotonic function on $\mathbb{R}$ is both quasiconvex and quasiconcave.
...
Thus,
$\nabla g(0,0)=(0,0)$, so the constraint qualification condition does not hold at $(0,0)$. 
...
&=1-2\lambda_1 x-\lambda_2+\lambda_3\\
&=0,
\end{aligned}
 \tag{1}
\]
...
\begin{aligned}
w_2
&=p\beta (t^\rho+1)^{\beta/\rho-1}x_2^{\beta-\rho}x_2^{\rho-1}\\
&=
...
\max_{\{v_i\}}
\left\{
\sum_{i=1}^N \lambda p_i f_i(v_i)
\;\middle|\;
\sum_{i=1}^N v_i \le V
\right\}.
\]
Since $\lambda$ is just a positive scalar, it can be factored out of the objective:
\[
\begin{aligned}
G(\lambda p, V)
&=
...
\end{aligned}
\]
Hence $G(p,V)$ is homogeneous of degree one in $p$.
...
\sum_{i=1}^N p_i f_i(v_i)
\;\middle|\;
\sum_{i=1}^N v_i \le \mu V
\right\}.
\]
Define $v_i=\mu z_i$. Then the constraint becomes
\[
\sum_{i=1}^N z_i \le V.
\]
Since each production function $f_i$ is homogeneous of degree one,
\[
f_i(v_i)=f_i(\mu z_i)=\mu f_i(z_i).
\]
Therefore,
\[
\begin{aligned}
...
\end{aligned}
\]
Hence $G(p,V)$ is homogeneous of degree one in $V$.
...
\right),
\]
where $w_j$ is the multiplier on factor constraint $j$.

By the envelope theorem, differentiating the value function with respect to output price $p_i$ gives
\[
...
y_i.
\]
Similarly, differentiating with respect to the endowment $V_j$ gives
\[
...
=
\frac{\partial^2 G}{\partial V_j \partial p_i}.
\]
If $G$ is twice continuously differentiable, Young's theorem implies equality of mixed partial derivatives:
\[
\frac{\partial^2 G}{\partial p_i \partial V_j}
=
\frac{\partial^2 G}{\partial V_j \partial p_i}.
...
\item[(iv)] \textbf{Restrictions on $\alpha,\beta,\gamma,\delta,\phi$.}

The translog GDP function is
\[
\begin{aligned}
\ln G
=
...
&
+ \frac12 \sum_{i=1}^N \sum_{j=1}^N \gamma_{ij}\ln p_i \ln p_j\\
&+ \frac12 \sum_{k=1}^M \sum_{l=1}^M \delta_{kl}\ln V_k \ln V_l\\
&+ \sum_{i=1}^N \sum_{k=1}^M \phi_{ik}\ln p_i \ln V_k.
\end{aligned}
\]

To be consistent with the theoretical properties above, the following restrictions are needed.
...
\textbf{(b) Linear homogeneity in prices $p$}

Since
\[
G(\lambda p,V)=\lambda G(p,V),
\]
we must have
\[
\ln G(\lambda p,V)=\ln \lambda+\ln G(p,V).
\]
This implies
\[
\sum_{i=1}^N \alpha_i = 1,
...
\textbf{(c) Linear homogeneity in factor endowments $V$}
...
\]
\[
\sum_{k=1}^M \delta_{kl}=0
\qquad
\forall l,
\]
and by symmetry also
\[
\sum_{l=1}^M \delta_{kl}=0
\qquad
\forall k,
\]
\[
\sum_{k=1}^M \phi_{ik}=0
\qquad
\forall i.
\]

Thus the full set of restrictions is
\[
\sum_{i=1}^N \alpha_i=1,
\qquad
\sum_{k=1}^M \beta_k=1,
...
\]
\[
\sum_{i=1}^N \gamma_{ij}=0 \quad \forall j,
\qquad
\sum_{j=1}^N \gamma_{ij}=0 \quad \forall i,
\]
\[
\sum_{k=1}^M \delta_{kl}=0 \quad \forall l,
\qquad
\sum_{l=1}^M \delta_{kl}=0 \quad \forall k,
\]
\[
\sum_{i=1}^N \phi_{ik}=0 \quad \forall k,
\qquad
\sum_{k=1}^M \phi_{ik}=0 \quad \forall i.
\]

\bigskip

\item[(v)] \textbf{Derive the Rybczynski elasticity}
\[
\frac{\partial \ln y_i}{\partial \ln V_j}.
\]
...
=
\frac{p_i}{G}\frac{\partial G}{\partial p_i}
=
\frac{p_i y_i}{G},
\]
define the output revenue share
\[
s_i \equiv \frac{p_i y_i}{G}.
\]
From the translog form,
\[
\frac{\partial \ln G}{\partial \ln p_i}
=
\alpha_i+\sum_{r=1}^N \gamma_{ir}\ln p_r+\sum_{k=1}^M \phi_{ik}\ln V_k.
\]
Hence
\[
s_i
...
Taking logs,
\[
\ln y_i=\ln G-\ln p_i+\ln s_i.
\]
Differentiate with respect to $\ln V_j$:
\[
\frac{\partial \ln y_i}{\partial \ln V_j}
=
\frac{\partial \ln G}{\partial \ln V_j}
+
\frac{1}{s_i}\frac{\partial s_i}{\partial \ln V_j}.
\]
But
\[
\frac{\partial \ln G}{\partial \ln V_j}
=
\frac{V_j}{G}\frac{\partial G}{\partial V_j}
=
\frac{V_j w_j}{G}.
\]
Define the factor income share
\[
m_j \equiv \frac{V_j w_j}{G}.
\]
Also,
\[
\frac{\partial s_i}{\partial \ln V_j}=\phi_{ij}.
\]
Therefore,
\[
\boxed{
...
\item[(vi)] \textbf{Derive the Stolper--Samuelson elasticity}
\[
\frac{\partial \ln w_j}{\partial \ln p_i}.
\]
...
m_j
=
\beta_j+\sum_{l=1}^M \delta_{jl}\ln V_l+\sum_{r=1}^N \phi_{rj}\ln p_r.
\]
Thus
\[
w_j=\frac{G}{V_j}m_j.
...
Taking logs,
\[
\ln w_j=\ln G-\ln V_j+\ln m_j.
\]
Differentiate with respect to $\ln p_i$:
\[
\frac{\partial \ln w_j}{\partial \ln p_i}
=
\frac{\partial \ln G}{\partial \ln p_i}
+
\frac{1}{m_j}\frac{\partial m_j}{\partial \ln p_i}.
\]
Now
\[
\frac{\partial \ln G}{\partial \ln p_i}=s_i,
\qquad
\frac{\partial m_j}{\partial \ln p_i}=\phi_{ij}.
\]
Hence
\[
\boxed{
\frac{\partial \ln w_j}{\partial \ln p_i}
=
s_i+\frac{\phi_{ij}}{m_j}
}.
\]

\bigskip

\item[(vii)] \textbf{Derive}
\[
\frac{\partial \ln w_j}{\partial \ln V_k}
\]
for $k=j$ and $k\neq j$.
...
\ln w_j=\ln G-\ln V_j+\ln m_j,
\]
differentiate with respect to $\ln V_k$:
\[
\frac{\partial \ln w_j}{\partial \ln V_k}
=
\frac{\partial \ln G}{\partial \ln V_k}
-
\frac{\partial \ln V_j}{\partial \ln V_k}
+
\frac{1}{m_j}\frac{\partial m_j}{\partial \ln V_k}.
\]
Now
\[
\frac{\partial \ln G}{\partial \ln V_k}=m_k,
\]
and
\[
m_j
...
so
\[
\frac{\partial m_j}{\partial \ln V_k}=\delta_{jk}.
\]
...
\frac{\partial \ln w_j}{\partial \ln V_k}
=
m_k-\mathbf{1}\{j=k\}+\frac{\delta_{jk}}{m_j},
\]
where $\mathbf{1}\{j=k\}$ is the indicator function.
...
\[
\boxed{
\frac{\partial \ln w_j}{\partial \ln V_j}
=
m_j-1+\frac{\delta_{jj}}{m_j}
}
\]
