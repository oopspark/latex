...
\[
t \frac{\partial f(t x_1, \dots, t x_N)}{\partial x_n}
- t^r \frac{\partial f(x_1, \dots, x_N)}{\partial x_n} = 0
\]

Thus,
\[
\frac{\partial f(t x_1, \dots, t x_N)}{\partial x_n}
...
Let
\[
x = \alpha (x+z) + (1-\alpha)x = x + \alpha z.
\]
...
\[
\alpha f(x+z) + (1-\alpha) f(x) < f(x),
\]
and thus $f$ is strictly concave. $\blacksquare$
...
Suppose that $f$ is concave. Consider $x$ and $x + \alpha z$, where $\alpha \in \mathbb{R}$.
...
= \frac{1}{2} \alpha^2 z^\top D^2 f(x + \beta z) z
\]
...
\[
z^\top D^2 f(x) z \le 0. \quad \blacksquare
\]
...
\[
\nabla f(x)\cdot (x' - x) > 0
\quad \text{whenever } f(x') \ge f(x), \ x' \ne x.
\]
...
\begin{itemize}
    \item $f(x)$ is concave $\iff z^\top D^2 f(x) z \le 0 \quad \forall z \in \mathbb{R}^N$
    \item $f(x)$ is strictly concave $\iff z^\top D^2 f(x) z < 0 \quad \forall z \ne 0$
\end{itemize}

\subsection*{Summary 2 (Quasiconcavity)}

\begin{itemize}
    \item $f(x)$ is quasiconcave $\iff z^\top D^2 f(x) z \le 0$
    whenever $\nabla f(x)\cdot z = 0$ and $\nabla f(x) \ne 0$
    \item $f(x)$ is strictly quasiconcave $\iff z^\top D^2 f(x) z < 0$
    whenever $\nabla f(x)\cdot z = 0$, $z \ne 0$
\end{itemize}
...
Here only the first $t \le T$ rows and $s \le S$ columns are retained.
...
Suppose that
\[
x=(x_1,\dots,x_N)\in A \subset \mathbb{R}^N
\quad \text{and} \quad
...
\det
\begin{pmatrix}
\frac{\partial f_1(x;\bar q)}{\partial x_1} & \cdots & \frac{\partial f_1(x;\bar q)}{\partial x_N} \\
\vdots & \ddots & \vdots \\
\frac{\partial f_N(x;\bar q)}{\partial x_1} & \cdots & \frac{\partial f_N(x;\bar q)}{\partial x_N}
\end{pmatrix}
\neq 0,
\]
then the system can be locally solved at $(x,\bar q)$ by implicitly defined
functions
\[
\eta_n : B' \to A'
\]
...
D_q \eta(q)
=
-\bigl[D_x f(x;\bar q)\bigr]^{-1} D_q f(x;\bar q).
\]
...
\[
D_x f(x;q)\, D_q \eta(q) + D_q f(x;q)=0.
\]

By the full-rank condition, $D_x f(x;q)$ is invertible. Therefore,
\[
D_q \eta(q)
=
-\bigl[D_x f(x;\bar q)\bigr]^{-1} D_q f(x;\bar q).
...
\frac{d\eta(q)}{dq}
=
-\frac{\partial f(x;q)/\partial q}{\partial f(x;q)/\partial x}.
\]
