...
A vector $\bar x \in \mathbb{R}^N$ is a \textbf{local maximizer} of $f(\cdot)$
...
Consider
\[
\phi(\varepsilon) = f(\bar x + \varepsilon z).
\]

A Taylor expansion around $\varepsilon=0$ gives
\[
f(\bar x+\varepsilon z) - f(\bar x)
=
\varepsilon \nabla f(\bar x)\cdot z
+ \frac{1}{2}\varepsilon^2 z^\top D^2 f(\bar x) z + R.
\]
...
If $\bar x$ is a local maximizer, then
\[
\begin{aligned}
&2 \frac{f(\bar x+\varepsilon z) - f(\bar x)}{\varepsilon ^2}\\
&= z^\top D^2 f(\bar x) z + 2 \frac{R}{\varepsilon ^2}\\
&\le 0.
\end{aligned}
\]
...
\[
z^\top D^2 f(\bar x) z \le 0.
\]
(Note that $R$ consists of higher-order terms $\varepsilon^3, \varepsilon^4, \dots$.
Hence $R$ becomes negligible as $\varepsilon \to 0$.)
\medskip

\textbf{(ii)} If
\[
z^\top D^2 f(\bar x) z < 0 \quad \forall z \ne 0,
\]
then for sufficiently small $\varepsilon > 0$,
\[
f(\bar x+\varepsilon z) < f(\bar x),
\]
so $\bar x$ is a local maximizer. \quad $\blacksquare$
...
$g_1, \dots, g_M$ are differentiable, and that $\bar x \in C$ is a local
...
Dg(\bar x) =
\begin{pmatrix}
\frac{\partial g_1}{\partial x_1}(\bar x) & \cdots & \frac{\partial g_1}{\partial x_N}(\bar x) \\
\vdots & \ddots & \vdots \\
\frac{\partial g_M}{\partial x_1}(\bar x) & \cdots & \frac{\partial g_M}{\partial x_N}(\bar x)
\end{pmatrix}
\]
has rank $M$.

Then there exist numbers (Lagrange multipliers)
$\lambda_1, \dots, \lambda_M \in \mathbb{R}$ such that
\[
\frac{\partial f}{\partial x_n}(\bar x)
=
\sum_{m=1}^M \lambda_m \frac{\partial g_m}{\partial x_n}(\bar x),
\qquad n = 1, \dots, N.
\]
...
If a pair $(\bar x,\bar \lambda)$ satisfies
\[
g_m(\bar x)=b_m \quad \text{and} \quad
\nabla f(\bar x)=\sum_{m=1}^M \lambda_m \nabla g_m(\bar x),
\]
we say that $(\bar x,\bar \lambda)$ satisfies the \textbf{first-order necessary conditions}
...
We prove the theorem using the \textit{Elimination Approach}, reducing the constrained
problem to an unconstrained one via the implicit function theorem.
...
\[
\max f(x_1,x_2)
\qquad \text{s.t.} \qquad g(x_1,x_2)=b.
\]
...
\[
f_1(\bar x)=\lambda g_1(\bar x),
\qquad
\lambda=\frac{f_1(\bar x)}{g_1(\bar x)}.
\]
...
Since $g_1 \neq 0$, we can solve locally for
\[
x_1=h(x_2),
\]
and obtain the unconstrained problem
\[
\max f(h(x_2),x_2).
\]
...
Differentiating the constraint $g(h(x_2),x_2)=b$ gives
\[
g_1\,h'(x_2)+g_2=0
\quad \Rightarrow \quad
h'(x_2)=-\frac{g_2}{g_1}.
...
Substituting,
\[
f_1\left(-\frac{g_2}{g_1}\right)+f_2=0
\quad \Rightarrow \quad
...
If $N=2$, $M=1$, and $b=0$, then the vector $\nabla f(\bar x)$ must lie on the same line as $\nabla g(\bar x)$ (i.e., $\nabla f(\bar x)=\lambda \nabla g(\bar x)$), where $x$ is the constrained maximum and the constraint is $g(x)=0$.

This is because the curve $g(x)=0$ is tangent at $\bar x$ to the level curve $f(x)=f(\bar x)$.
The vector $\nabla g(\bar x)$ is perpendicular at $\bar x$ to the curve $g(x)=0$, and the vector $\nabla f(\bar x)$ is perpendicular at $\bar x$ to the curve $f(x)=f(\bar x)$.

\medskip

Why is the gradient $\nabla f(x)$ perpendicular to its level curve?
Consider a level curve $k=f(x_1,x_2)$ and differentiate it implicitly, taking $x_2$ as a function of $x_1$:
\[
...
On the other hand, the slope of the line in the direction of the gradient is
\[
\frac{\Delta x_2}{\Delta x_1} = \frac{f_2(x_1,x_2)}{f_1(x_1,x_2)}.
\]

Therefore, the gradient $\nabla f(x)$ is perpendicular to the tangent line.

\medskip

For a more general case where $N \ge 3$ and $M \ge 2$, the theorem requires that the gradient vector $\nabla f(\bar x)$ belongs to the subspace spanned by the vectors $\nabla g_m(\bar x)$, $m=1,\dots,M$:
\[
\begin{aligned}
&\mathrm{span}\big(\nabla g_1(x),\dots,\nabla g_M(x)\big)\\
&=
\left\{
z \in \mathbb{R}^N : z = \sum_{m=1}^M \alpha_m \nabla g_m(x),\ \alpha_m \in \mathbb{R}
\right\}.
\end{aligned}
\]
...
The Theorem of Lagrange implies that if $\bar x$ is a local constrained maximizer
...
\subsection*{Theorem 13 (Second-Order Sufficient Condition)}

Suppose that at $\bar x$ the constraint qualification is satisfied and that
there exist Lagrange multipliers $\lambda_1,\dots,\lambda_M$ such that
\[
\nabla f(\bar x) = \sum_{m=1}^M \lambda_m \nabla g_m(\bar x).
\]
...
D^2 f(\bar x)
-
\sum_{m=1}^M \lambda_m D^2 g_m(\bar x).
\]

If $D^2 L(\bar x,\lambda)$ is negative semidefinite on the subspace
\[
\{ z \in \mathbb{R}^N : \nabla g_m(\bar x)\cdot z = 0 \ \text{for all } m \},
\]
then $\bar x$ satisfies the second-order necessary condition.

If, moreover, $D^2 L(\bar x,\lambda)$ is negative definite on the subspace
\[
\{ z \in \mathbb{R}^N : \nabla g_m(\bar x)\cdot z = 0,\ z \ne 0 \ \text{for all } m \},
\]
then $\bar x$ is a local maximizer.
...
The second-order condition (SOC) for the case $N=2$ and $M=1$ can be expressed as
\[
\begin{vmatrix}
L_{11} & L_{12} & -g_1 \\
L_{21} & L_{22} & -g_2 \\
...
A formal proof for the SOCs is given in the appendix based on the penalty approach.

\medskip

Assume that $N=2$, $M=1$, and both $f(x)$ and $g(x)$ are increasing in $x$.
In order to obtain a \textbf{maximum}, the level curve of $f(x)$ should be more convex
(or less concave) than that of $g(x)$.

\medskip

We derive the slope and curvature of the contour of $f(x)$.
...
Similarly, for the contour of $g(x)$,
\[
\frac{dx_2}{dx_1}
=
...
\[
\frac{d^2 x_2}{dx_1^2}\Big|_{f}
>
\frac{d^2 x_2}{dx_1^2}\Big|_{g},
\]
that is,
\[
-\frac{
...
- 2 g_1 g_2 (\lambda g_{12} - f_{12})
+ g_1^2 (\lambda g_{22} - f_{22})
> 0.
\]
...
We say that the \textbf{constraint qualification} is satisfied at $\bar x \in C$
if the constraints that hold with equality at $\bar x$ are linearly independent.
...
Suppose that $\bar x \in C$ is a local maximizer and that the constraint qualification is satisfied.
...
The Kuhn--Tucker theorem says that if $\bar x$ is a local constrained maximizer
...
-
\sum_{k=1}^K \mu_k \frac{\partial h_k(\bar x)}{\partial x_n}
=0
\]
...
and
\[
\mu_k\bigl(c_k-h_k(\bar x)\bigr)=0
...
\Phi(x,y,\mu)
=
f(x)+\sum_{k=1}^K \mu_k\bigl(c_k-h_k(x)-y_k^2\bigr).
\]
...
\sum_{k=1}^K \mu_k \frac{\partial h_k(x)}{\partial x_n}
=0\\
&\text{for every } n=1,\dots,N,
\end{aligned}\]
\[
\frac{\partial \Phi}{\partial y_k}=-2\mu_k y_k=0,
\]
and
\[
\begin{aligned}
...
\[
\begin{array}{c|c}
\text{Case} & \text{Interpretation} \\
\hline
\text{(a) } x>0 & \text{Interior optimum} \\
\text{(b) } x=0,\ \mu>0 & \text{Boundary maximum (binding)} \\
\text{(c) } x=0,\ \mu=0 & \text{Boundary with flat slope}
\end{array}
\]
...
\begin{itemize}
    ...
    Substituting into (1),
    \[
    \frac{a}{x} - \frac{p_x}{p_y} + \mu_x = 0,
    \]
    which implies $\frac{a}{x} = \infty \le \frac{p_x}{p_y}$, hence not feasible.
...
Let $F(v)$ be the cumulative distribution of values $v \in (0,\bar v)$,
and let $N$ be the number of bidders.

\medskip

Each bidder privately observes their value $v$ and chooses a bid $b$.
In equilibrium, we look for a \textit{bid function} $b(v)$, which specifies
the optimal bid for each value $v$.

\medskip

Since all bidders use the same increasing bid function $b(v)$,
a bidder wins if and only if their value is the highest.
...
The payoff of a bidder with value $v$ who bids $b(v)$ is
\[
...
\textit{Key idea:} Instead of solving directly for $b(v)$,
we first characterize the payoff function $\pi(v)$ using
...
Integrating,
\[
\pi(v) = \int_0^v F(x)^{N-1}\,dx,
\]
with $\pi(0)=0$ (a bidder with $v=0$ never wins).

\medskip

Now, using the definition of payoff,
\[
\pi(v) = F(v)^{N-1}\,[v - b(v)],
\]
we can solve for the bid function:
\[
b(v) = v - \frac{\pi(v)}{F(v)^{N-1}}
= v - \frac{\int_0^v F(x)^{N-1}\,dx}{F(v)^{N-1}}.
\]

\medskip

\textbf{Uniform case:} If $F(v)=\frac{v}{\bar v}$, then
\[
b(v) = \frac{N-1}{N}v.
\]