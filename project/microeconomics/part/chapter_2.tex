...
\subsection*{1.1 Production Possibilities Set $T$}
...
\begin{itemize}
    ...
    \hfill (free disposability of inputs).
    
    \item[T.5] If $(x,y) \in T$ and $y' \le y$, then $(x,y') \in T$
    \hfill (free disposability of outputs).
    
    \item[T.6] For every finite $x$, the set of feasible outputs is bounded above.
    
    \item[T.7] $(x,0) \in T$, but if $y \ne 0$, then $(0,y) \notin T$
    \hfill (weak essentiality / no free lunch).
\end{itemize}
...
\begin{itemize}
    ...
    \quad \alpha \in [0,1].
    \]
    That is, convex combinations are feasible.
\end{itemize}
...
Property T.3 is closely related to the concept of a diminishing marginal
rate of technical substitution (MRTS).

\medskip

\textbf{Further interpretation of properties:}
...
\subsection*{1.2 Input Requirement Set $V(y)$}
...
\begin{itemize}
    ...
    \item[V.5] If $y' \le y$, $y \ge 0$, then
    \[
    V(y') \subseteq V(y).
    \]
\end{itemize}
...
\textbf{Additional property:}

\begin{itemize}
    \item[V.6] $0 \in V(0)$
    \hfill (follows from T.7: zero output is always producible).
\end{itemize}
...
\subsection*{1.3 Producible-Output Set $Y(x)$}
...
\subsection*{1.4 Production or Transformation Function}

\subsection*{1.4.1 A Production Function}
...
\subsection*{1.4.2 A Transformation Function}
...
\subsection*{1.5 Output and Input Distance Functions}

\subsection*{1.5.1 Output Distance Function}
...
From the properties of the output distance function,
\[
D^O(x,y+y') \le D^O(x,y) + D^O(x,y').
\]

Let $y = \lambda y$ and $y' = (1-\lambda)y'$. Then,
\[
D^O(x, \lambda y + (1-\lambda)y')
\le
\lambda D^O(x,y) + (1-\lambda)D^O(x,y'),
\]
which shows convexity of $D^O(x,y)$ in $y$.
\hfill $\blacksquare$

\subsection*{1.5.2 Input Distance Function}
...
Note that
\[
x \in V(y) \;\Longleftrightarrow\; D^I(x,y) \ge 1.
\]
...
\begin{itemize}
    ...
    \item[Di.4] 
    \[
    D^I(\mu x,y)=\mu D^I(x,y), \qquad \mu>0
    \]
    (homogeneous of degree 1 in $x$).

    \item[Di.5] 
    \[
    D^I(\lambda x+(1-\lambda)x',y)
    \ge
    \lambda D^I(x,y)+(1-\lambda)D^I(x',y)
    \]
    (concave in $x$).
\end{itemize}
...
\subsection*{Proof of the CRS Relationship}
...
\[
D^{\rightarrow}(x,y;g_x,g_y) = -\infty.
\]

\medskip

Here $(g_x,g_y)$ is a reference direction for inputs and outputs.
The function $D^{\rightarrow}(x,y;g_x,g_y)$ represents the maximal translation
...
\begin{itemize}
    \item[c.1] $c(w,y) > 0$ for $y>0$ \hfill (nonnegativity)

    \item[c.2] If $w' \ge w$, then
    \[
    c(w',y) \ge c(w,y)
    \]
    \hfill (nondecreasing in $w$)
    ...
    \quad t>0
    \]
    \hfill (homogeneous of degree 1 in $w$)

    \item[c.5] If $y \ge y'$, then
    \[
    c(w,y) \ge c(w,y')
    \]
    \hfill (nondecreasing in $y$)

    \item[c.6] 
    \[
    c(w,0)=0
    \]
    \hfill (no fixed cost; not required in general)

    \item[c.7] (\textbf{Shephard’s Lemma})
    If $c(w,y)$ is differentiable at $(w,y)$ and $w_i>0$, then
    \[
...
\begin{itemize}
    \item[c.2] If $x,x'$ minimize cost at $w,w'$ respectively, then
    $wx \le wx'$ and since $w \le w'$, we get $wx' \le w'x'$.

    \item[c.3] Let $w''=tw+(1-t)w'$. Then
    \[
    c(w'',y)=w''x'' = t w x'' + (1-t) w' x''
    \]
    and use optimality.

    \item[c.4] Scaling argument: minimizing at $w$ is equivalent to minimizing at $tw$.

    \item[c.5] If $y \ge y'$, then $V(y) \subseteq V(y')$.

    \item[c.7] Apply the envelope theorem to
    \[
    \mathcal{L}=w\cdot x + \lambda Y(x,y).
    \]
\end{itemize}
...
Derive a cost function consistent with the CES production function
\[
...
If output is one-dimensional and there are constant returns to scale, show that
\[
c(w,y)=y\,c(w,1).
\]
...
\subsection*{2.2 Revenue Function}
...
\begin{itemize}
    ...
    \qquad t>0
    \]
    \hfill (homogeneous of degree 1 in $p$)

    \item[R.5] If $x' \ge x$, then
    \[
    R(p,x') \ge R(p,x)
    \]
    \hfill (nondecreasing in $x$)
\end{itemize}
...
\subsection*{Example 3 (Linear Technology)}
...
c(w,y)
=
\min_x \left\{ \frac{w\cdot x}{D^I(x,y)} \right\},
\]
...
\[
D^I(x,y)=\frac{x}{\sqrt{y_1^2+y_2^2}}.
\]
...
R(p,x)
=
\max_y \left\{ \frac{p\cdot y}{D^O(x,y)} \right\},
\]
...
\[
D^O(x,y)=\max_p \frac{p\cdot y}{R(p,x)}.
\]
...
Thus,
\[
\max \theta = OD = \frac{OB}{OC} = p^D \cdot y^B = D^O(x,y).
\]

This explains why
\[
D^O(x,y)=\max_p \{p\cdot y : R(p,x)=1\}.
\]
...
\[
\mathcal{L}=p\cdot y + \lambda(1 - D^O(x,y)).
\]
...
\frac{p_2}{p_1}
=
\frac{\partial D^O(x,y)/\partial y_2}
{\partial D^O(x,y)/\partial y_1},
\]
